...
namespace BGLib.LowEnergy
{
    public class Central : IDisposable
    {
        ...
        public Central(string portName, int baudRate, Parity parity, int dataBits, StopBits stopBits)
        {
            ...
            _messageHub = new MessageHub(_communicator);
        ...
        private void OnScanResponse(object sender, ScanResponseEventArgs e)
        {
            ...
            var eventArgs = new DiscoveryEventArgs(type, address, e.Data, e.RSSI);
        ...
        public async Task StartDiscoveryAsync(DiscoverMode mode = DiscoverMode.Generic, DiscoverSettings settings = null)
        {
            ...
            await _messageHub.GAP.DiscoverAsync(mode);
        }
        ...
        public async Task<Peripheral> ConnectAsync(Address address)
        {
            var connectionTCS = new TaskCompletionSource<byte>();
            var onStatus = new EventHandler<StatusEventArgs>((s, e) =>
            {
                var eAddress = new Address(e.AddressType, e.Address);
                var equals = Equals(eAddress, address);
                if (!equals)
                    return;
            ...
            try
            {
                var rawValue = address.RawValue;
                var type = address.Type;
                await _messageHub.GAP.ConnectDirectAsync(rawValue, type, 0x20, 0x30, 0x100, 0);
                var connection = await connectionTCS.Task;
                var peripheral = new Peripheral(connection, address);
